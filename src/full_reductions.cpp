...
void full_reductions::reduce_graph() {
	...
	std::cout << "Before first call to reduce_graph" << std::endl;
	parallel_reducers.back()->reduce_graph(mis_config.number_of_partitions, mis_config.partitioner, mis_config.weightType);
	std::cout << "After first call to reduce_graph" << std::endl;
	std::cout << "Kernel size after parallel run: " << parallel_reducers.back()->size() << std::endl;
	std::cout << "Before call to updateAllNeighborhoods" << std::endl;
	parallel_reducers.back()->updateAllNeighborhoods();
	std::cout << "After call to updateAllNeighborhoods" << std::endl;
	std::cout << "Before second call to reduce_graph" << std::endl;
	parallel_reducers.back()->reduce_graph(1, mis_config.partitioner, mis_config.weightType);
	std::cout << "After second call to reduce_graph" << std::endl;
