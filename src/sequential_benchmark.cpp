...
int main(int argn, char **argv) {
    ...
    std::unique_ptr<branch_and_reduce_algorithm> full_reducer = std::unique_ptr<branch_and_reduce_algorithm>(new branch_and_reduce_algorithm(adj, adj.size()));

    double begin, end, elapsed_secs;
    begin = omp_get_wtime();
    full_reducer->reduce_graph_few_reductions();
    end = omp_get_wtime();
    elapsed_secs = double(end - begin);
    std::cout << "Sequential took " << elapsed_secs << " seconds" << std::endl;

    auto is_base = full_reducer->get_current_is_size_with_folds();
    mis_log::instance()->print_reduction(mis_config, is_base, full_reducer->number_of_nodes_remaining());
