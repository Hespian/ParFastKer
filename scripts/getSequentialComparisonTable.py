...
def getOurTimeAndSize(inputFile):
    ...
    return (parallel_time[1], parallel_size[1])

ourResultsDir = sys.argv[1]
akibaResultsDir = sys.argv[2]

latexFileName = os.path.join(akibaResultsDir, "sequentialComparison.tex")
...
latexFile.write("\\begin{tabular}{l|rr|rrr|rr}\n")
latexFile.write(" & \multicolumn{2}{c|}{Akiba and Iwata \cite{akiba2016branch}} & \multicolumn{3}{c|}{Our algorithm} & \multicolumn{2}{c}{Same size comparison} \\\\ \n")
latexFile.write("graph & size & time [s] & size & time [s] & speedup & time [s] & speedup \\\\ \n")
latexFile.write("\\hline \n")

for filename in os.listdir(ourResultsDir):
    if filename.endswith(".graph"):
        ourtime, oursize = getOurTimeAndSize(os.path.join(ourResultsDir, filename))
