...
def getOurTimeAndSize(inputFile):
    ...
    for line in file:
        ...
        if parallel:
            ...
            if "Kernel size after sequential run:" in line:
                sequential_size[num_blocks] += int(words[-1])
    ...
    if not sizes[-1] == 64:
        is64 = False
    return (sequential_time[sizes[-1]] + parallel_time[sizes[-1]], sequential_size[sizes[-1]], partitioning_time, is64)

ourResultsDir = sys.argv[1]
akibaResultsDir = sys.argv[2]

latexFileName = os.path.join(akibaResultsDir, "parallelExactKernelComparison.tex")
