...
plt.xlabel('# Partitions')
plt.ylabel('# Reductions')
plt.title('Number of unconfined reductions performed in parallel')
plt.xticks(index, sizes)
...
if not latexFileName == None:
	...
	latexFile.write("\\newline \n")
	latexFile.write("\\begin{tabular}{l|rrrrrrr|r}\n")
	latexFile.write("number of blocks & parallel run time & parallel kernel size & vertex folds & isolated clique reductions & twin reductions (removed) & twin reductions (folded) & unconfined reductions & sequential run time \\\\ \n")
	latexFile.write("\\hline \n")
	for i in range(len(sizes)):
		latexFile.write(str(sizes[i]) + " & " + str(round(parallel_times[i], 1)) + " & " + str(int(parallel_sizes[i])) + " & " + str(int(sum_vertex_folds[i])) + " & " + str(int(sum_isolated_clique[i])) + " & " + str(int(sum_twins_removed[i])) + " & " + str(int(sum_twins_folded[i])) + " & " + str(int(sum_unconfined[i])) + " & " + str(round(sequential_times[i], 1)) + "\\\\ \n")
