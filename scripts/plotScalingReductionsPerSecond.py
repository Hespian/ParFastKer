...
def readGraphFile(inputFile, color):
	...
	for line in file:
		...
		if "Total time spent undoing" in line:
			if block_sizes[num_blocks][num_blocks - 1] == 0:
				raise ValueError(graphName + "(" + str(num_blocks) + "): Last block is empty")
		...
		if "num threads:" in line:
			...
			if not num_threads == num_blocks:
				raise ValueError(str(num_threads) + " != " + str(num_blocks))
		...
		if "Before call to sequential reduce_graph" in line:
			parallel = False
		if parallel:
			if line.strip().endswith("vertices"):
				block_num = int(words[0][:-1])
				block_sizes[num_blocks][block_num] = int(words[1])
	...
	reductions_per_second_rel = [0] * len(sizes)
	reductions_per_second_base = ( numVertices - parallel_size[1]) / parallel_time[1]
	for i in range(len(sizes)):
		...
		reductions_per_second_rel[i] = ((numVertices - parallel_size[sizes[i]]) / parallel_time[sizes[i]]) / reductions_per_second_base

	plt.semilogx(sizes, reductions_per_second_rel, color=color, label=graphName, basex=2, marker="x")
...
filename = os.path.join(directory, "reductionsPerSecond")
...
plt.ylabel('Vertices removed per second (relative)')
