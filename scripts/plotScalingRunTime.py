...
def readGraphFile(inputFile, color, axes):
    ...
    for line in file:
        ...
        if "Number of blocks:" in line:
            ...
            LP_time[num_blocks] = 0
            Rest_time[num_blocks] = 0
    ...
    speedup = [0] * len(sizes)
    speedup_LP = [0] * len(sizes)
    speedup_rest = [0] * len(sizes)
    for i in range(len(sizes)):
        parallel_times[i] = parallel_time[sizes[i]]
        speedup[i] = parallel_time[min(parallel_time)] / parallel_time[sizes[i]]
        speedup_rest[i] = Rest_time[min(Rest_time)] / Rest_time[sizes[i]]
        speedup_LP[i] = LP_time[min(LP_time)] / LP_time[sizes[i]]

    axes[0].semilogx(sizes, speedup, color=color, label=graphName, basex=2, marker="x")
    axes[1].semilogx(sizes, speedup_LP, color=color, label=graphName, basex=2, marker="o")
    axes[2].semilogx(sizes, speedup_rest, color=color, label=graphName, basex=2, marker=".")
    return sizes
...
filename = os.path.join(directory, "runtime")
...
width = 0.75
cmapIndex = 1
f, axes = plt.subplots(3, figsize=(8,20))
axes[0].set_title("Overall")
axes[1].set_title("LP")
axes[2].set_title("Rest")
for resultsFile in os.listdir(directory):
    ...
    if resultsFile.endswith(".graph") and os.path.isfile(resultsFilePath):
        sizes = readGraphFile(resultsFilePath, cmap(cmapIndex), axes)
        cmapIndex += 1
...
pp = PdfPages(filenamepdf)
plt.savefig(filenamepdf, format='pdf')
